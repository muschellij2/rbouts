...
nonWear= function(df) {
  ...
  #' @export
  df %>% mutate(.criterioRaw=.criterioNW,.criterioBout=.criterioRaw) %>%
...
MVPA_B1=function(df) {
  ...
  #'
  #'
  df %>% mutate(.criterioRaw =criterioENMO(.,limInf = 100/1000 )) %>%
    mutate(.criterioBout=criterioBout(.,pctBouts = 0.8,durBoutMin = dminutes(1))) %>%
    getSummary()
}
...
LPA_B1=function(df) {
  ...
  #'
  df %>%
    mutate(.criterioRaw=criterioENMO (.,limInf=100/1000)) %>%
    mutate(.criterioBout=criterioBout(.,pctBouts = 0.8,durBoutMin = dminutes(1))) %>%
    mutate(.criterioRaw=!.criterioBout &  criterioENMO(.,limInf=40/1000)) %>%
    mutate(.criterioBout=criterioBout(.,pctBouts = 0.8,durBoutMin = dminutes(1))) %>%
    getSummary()
}
...
INA_B1=function(df) {
  ...
  #'
  df %>%
    mutate(.criterioRaw= criterioENMO(.,limSup=40/1000)) %>%
...
INA_B30=function(df) {
  ...
  #'
  df %>%
    mutate(.criterioRaw=criterioENMO (.,limInf=40/1000)) %>%
    mutate(.criterioBout=criterioBout(.,pctBouts = 1,durBoutMin=dseconds(30))) %>%
    mutate(.criterioRaw=!.criterioBout &  criterioENMO(.,limSup=40/1000)) %>%
...
OINA_B1=function(df) {
  ...
  df %>%
    ...
    mutate(.criterioRaw= !.criterioRaw & criterioENMO(.,limSup=40/1000)) %>%
    mutate(.criterioBout=criterioBout(.,pctBouts = 0.95,durBoutMin = dminutes(1))) %>%
    getSummary()
}

OINA_B30=function(df) {
  #' Summarises Inactivity bouts (INA, <1.5METs). But no SIBs are included. Activities such as lying, sitting, standing, moving slowly.
...
MuyQuieto=function(df) {
  ...
  #' @export
  #'
  df %>% mutate(.criterioRaw=criterioENMO(.,limSup = 15/1000,useNW=FALSE)) %>%
